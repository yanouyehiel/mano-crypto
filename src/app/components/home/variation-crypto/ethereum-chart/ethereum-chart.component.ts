...
@Component({
  selector: 'app-ethereum-chart',
  templateUrl: './ethereum-chart.component.html'
})
export class EthereumChartComponent implements OnInit {

  @ViewChild(BaseChartDirective) chart: BaseChartDirective | undefined;

  requestParams = { symbol: 'ETHUSDT', interval: '1s', limit: '2000', byRange: false, startTime: '', endTime: '' }
    ...
    datasets: [
      {
        data: [], borderWidth: 1,
        pointRadius: 0,
        label: `Ethereum(USDT)`, borderColor: '#8585ff', tension: 0.1
      }
    ]
  };
  ...
  startRealTimeUpdates(): void {
    this.cryptoService.getCryptoData('ETHUSDT').subscribe((message) => {
  ...
  updateChartRange(range: string) {
    ...
    if (this.timeRange === '7d') {
      ...
      this.loadHistoricalData({
        ...this.requestParams, byRange: false, interval: '1s',// Utiliser des bougies de 1 heure pour 1 mois
        limit: 2000
      })
    }
  }
  ...
  customizeRange(startDate: string, endDate: string) {
    ...
    this.loadHistoricalData({
      ...
      limit: 2000, byRange: true, startTime: ((new Date(startDate).getTime()) / 1000).toString(), endTime: ((new Date(endDate).getTime()) / 1000).toString()
    })
